...
const FinancialScreen = () => {
  ...
  const [subscription, setSubscription] =
    useState<SupplierSubscriptionData | null>(null);
  ...
  const [selectedReceipt, setSelectedReceipt] = useState<LedgerEntry | null>(
    null,
  );
...
Este é um comprovante digital gerado pelo sistema PMS.
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const financialRes = await api.get(
        `/financial/supplier/${activeSupplierId}`,
      );
  ...
  const openPlanModal = async () => {
    ...
    try {
      ...
      if (supplier?.plan?.id) {
        const currentPlan = loadedPlans.find(
          (p: any) => p.id === supplier.plan?.id,
        );
        if (currentPlan) {
      ...
      Alert.alert(
        "Erro",
        `Não foi possível carregar os planos: ${e.message || "Erro desconhecido"}`,
      );
    } finally {
  ...
  const confirmChangePlan = async () => {
    ...
    if (!supplier) {
      Alert.alert(
        "Erro",
        "Dados do fornecedor não carregados. Tente recarregar a tela.",
      );
      return;
    }
  ...
  const processPayment = async (method: "BALANCE" | "CARD") => {
    ...
    if (
      method === "BALANCE" &&
      supplier.walletBalance < supplier.plan.monthlyPrice
    ) {
      Alert.alert(
        "Saldo Insuficiente",
        "Você não tem saldo suficiente na carteira para realizar este pagamento.",
      );
      return;
    }
  ...
  const handleWithdraw = () => {
    ...
    if (isNaN(amount) || amount <= 0) {
      Alert.alert(
        "Valor Inválido",
        "Por favor, insira um valor válido para saque.",
      );
      return;
    }
    ...
    if (limits) {
      if (amount < limits.min) {
        Alert.alert(
          "Valor Mínimo",
          `O valor mínimo para saque é R$ ${limits.min.toFixed(2)}.`,
        );
        return;
      }
      if (limits.remaining <= 0) {
        Alert.alert(
          "Limite Atingido",
          `Você atingiu o limite mensal de ${limits.limitCount} saques.`,
        );
        return;
      }
    }
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                ...
                "Seu saque foi processado e será enviado em breve.",
              );
            } catch (e) {
              console.log(e);
              Alert.alert(
  ...
  const handleRefetchUser = async () => {
    ...
    try {
      await refetchUser();
      // After refetching user, activeSupplierId should update in context
      // The useEffect dependent on activeSupplierId will trigger loadData automatically
    } catch (e) {
      Alert.alert(
        "Erro",
        "Não foi possível atualizar o cadastro. Tente sair e entrar novamente.",
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <SafeAreaView style={styles.container} edges={["top"]}>
        ...
        <View style={[styles.content, styles.center]}>
          ...
          >
            {errorState ? "Erro ao carregar dados" : "Configuração Pendente"}
          </Text>
          <Text
            style={{
              color: colors.textSecondary,
          ...
          >
            {errorState
              ...
              : "Identificamos que sua conta precisa de uma atualização rápida para ativar o módulo financeiro."}
          </Text>

          {!errorState && !supplier && (
            <TouchableOpacity
              style={[styles.payButton, { marginTop: 24, width: "100%" }]}
              onPress={handleRefetchUser}
            >
              <Text style={styles.payButtonText}>Atualizar Cadastro</Text>
              <Ionicons
                name="refresh"
                size={20}
                color="#FFF"
                      ...
                      >
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={styles.planCycle}>
                            Ciclo: {p.cycleDays} dias • Comissão:{" "}
                            {p.commissionPercent}%
                          </Text>
                          <Text style={styles.planCycle}>
                            Limite Pedidos: {p.limitOrders} • Limite Produtos:{" "}
                            {p.limitProducts}
                          </Text>
                          <Text style={styles.planCycle}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      >
        ...
        <View style={styles.walletCard}>
          ...
          <View style={styles.walletFooter}>
            <Text style={styles.walletSubtext}>
              ...
              {subscription
                ? `Comissão ${subscription?.plan?.commissionPercent ?? 10}%`
                : ""}
            </Text>
            <View
              style={[
        ...
        {/* Pending Releases */}
        {ledger.some(
          (item) => item.status === "PENDING" && item.releaseDate,
        ) && (
                  ...
                  return (
                    <View
                      ...
                      style={{
                        ...
                        paddingVertical: 14,
                        borderBottomWidth:
                          index < pendingItems.length - 1 ? 1 : 0,
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          >
                            {isToday
                              ? "Libera Hoje"
                              : `Libera em ${releaseDate.toLocaleDateString("pt-BR")}`}
                          </Text>
                        </View>
                      </View>
        ...
        <View style={styles.card}>
          <View style={styles.subscriptionRow}>
            ...
            <View style={{ alignItems: "flex-end" }}>
              <Text style={styles.subscriptionPrice}>
                R$ {supplier?.plan?.monthlyPrice.toFixed(2)}
              </Text>
          ...
          {supplier?.financialStatus !== "ACTIVE" ||
          (supplier?.nextBillingDate &&
            new Date(supplier.nextBillingDate) < new Date()) ||
              ...
              <View key={item.id} style={styles.transactionItem}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor: isCredit(item.type)
                        ? "#E8F5E9"
                        : "#FFEBEE",
                    },
                  ]}
                ...
                <Text
                  style={[
                    ...
                    {
                      color: isCredit(item.type)
                        ? colors.success
                        : colors.error,
                    },
                  ]}
                >
                  {isCredit(item.type) ? "+" : "-"} R${" "}
                ...
                <View key={"modal-" + item.id} style={styles.transactionItem}>
                  ...
                  >
                    <Ionicons
                      ...
                      size={20}
                      color={
                        isCredit(item.type) ? colors.success : colors.error
                      }
                    />
                  </View>
                  <View style={styles.transactionContent}>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {addCardMode ? "Novo Cartão" : "Meus Cartões"}
              </Text>
              <TouchableOpacity
                onPress={() => {
                  setCardsModalVisible(false);
                ...
                <ScrollView style={{ maxHeight: 300 }}>
                  {plans.map((p) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.planRow,
                        selectedPlanId === p.id ? styles.planRowSelected : null,
                      ]}
                      onPress={() => setSelectedPlanId(p.id)}
                    >
                      <View style={{ flex: 1 }}>
            ...
            <View style={{ paddingVertical: 10 }}>
              <View style={{ flexDirection: "row", marginBottom: 16 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    Os valores das vendas ficam pendentes por um período de
                    segurança (geralmente 14 a 30 dias após a entrega), conforme
                    seu plano.
                  </Text>
                </View>
              </View>
              <View style={{ flexDirection: "row" }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    Isso garante que, em caso de devolução ou disputa, o valor
                    possa ser estornado sem prejudicar seu saldo negativo.
                  </Text>
                </View>
              </View>
            </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { width: "90%", maxWidth: 400 }]}>
                ...
                >
                  <View
                    style={{
                      ...
                      height: "100%",
                      backgroundColor:
                        limits.remaining > 0 ? colors.success : colors.error,
                    }}
                  />
                </View>
                <Text
            ...
            >
              ...
              <TouchableOpacity
                onPress={() =>
                  setWithdrawAmount(supplier?.walletBalance.toString() || "0")
                }
              >
                <Text
                  style={{
            ...
            <TouchableOpacity
              style={[
                ...
                parseFloat(withdrawAmount || "0") < (limits?.min || 0)
                  ? { backgroundColor: "#ccc" }
                  : {},
              ]}
