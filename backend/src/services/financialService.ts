...
export const FinancialService = {
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (commission === 0 && netValue === 0) {
        const commissionRate =
          await CommissionService.getCommissionRateForSupplier(
            order.supplierId,
          );
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const releasableItems = await tx.financialLedger.findMany({
        where: {
          ...
          releaseDate: { lte: now },
          type: {
            in: ["ORDER_CREDIT_PENDING", "SALE_REVENUE", "ORDER_REFUND_OFFSET"],
          }, // Release both new and legacy types
        },
      });
  ...
  getSupplierFinancials: async (supplierId: string): Promise<any> => {
    ...
    const minWithdrawal =
      supplier.plan?.minWithdrawal || settings?.defaultMinWithdrawal || 50;
    const limitCount =
      supplier.plan?.withdrawalLimit || settings?.defaultWithdrawalLimit || 4;
  ...
  requestWithdrawal: async (
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const settings = await tx.financialSettings.findUnique({
        where: { id: "global" },
      });
      const minWithdrawal =
        supplier.plan?.minWithdrawal || settings?.defaultMinWithdrawal || 50;
      ...
      if (amount < minWithdrawal) {
        throw new Error(
          `O valor mínimo para saque é R$ ${minWithdrawal.toFixed(2)}`,
        );
      }
      ...
      if (withdrawalsThisMonth >= withdrawalLimit) {
        throw new Error(
          `Limite mensal de saques atingido (${withdrawalLimit}).`,
        );
      }

      if (supplier.verificationStatus !== "VERIFIED") {
        throw new Error(
          "Conta não verificada. Complete a verificação para sacar.",
        );
      }
      ...
      if (!activeSub) {
        throw new Error(
          "Plano vencido ou inativo. Renove para realizar saques.",
        );
      }
  ...
  getAdminDashboard: async (): Promise<any> => {
    ...
    return {
      revenue: {
        ...
        total:
          (totalCommission._sum.amount || 0) +
          (totalSubscription._sum.amount || 0),
      },
      payouts: {
      ...
      balance: {
        totalHeld:
          (totalPendingBalance._sum.walletBalance || 0) +
  ...
  getAdminSupplierFinancials: async (
    search?: string,
    statusFilter?: string,
  ): Promise<any[]> => {
    const where: any = {};
    ...
    const commissionMap = new Map(
      commissions.map((c) => [c.supplierId, c._sum.amount || 0]),
    );
  ...
  getWithdrawalRequests: async (
    ...
    (WithdrawalRequest & {
      supplier: { name: string; billingDoc: string | null };
    })[]
  > => {
  ...
  processOrderPayment: async (orderId: string): Promise<Order> => {
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const releaseDays =
        order.supplier.plan?.releaseDays || settings?.defaultReleaseDays || 14;
      const releaseDate = new Date();
      releaseDate.setDate(releaseDate.getDate() + releaseDays);
      ...
      try {
        notificationService.notify(
          "Receita Pendente",
          `Pedido #${order.orderNumber} processado. R$ ${order.netValue} pendente.`,
          {
            orderId,
            amount: order.netValue,
          },
  ...
  processSubscriptionPayment: async (
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (method === "BALANCE") {
        ...
        // Use StripeService to create and confirm payment
        // We pass confirm: true to attempt immediate charge
        const paymentIntent = await StripeService.createPaymentIntent(
          amountInCents,
          "brl",
          undefined, // customerId not yet managed here
          paymentToken,
          { supplierId: supplierId, purpose: "SUBSCRIPTION" },
          true, // confirm immediately
          "http://localhost:3000/payment/callback", // return_url required for confirm: true
        );
      ...
      await tx.financialLedger.create({
        data: {
          ...
          description:
            method === "BALANCE"
              ? "Pagamento de Mensalidade (Saldo)"
              : `Pagamento de Mensalidade (Cartão - ${paymentToken?.slice(0, 15)}...)`,
          status: "COMPLETED",
        },
      });
